...
set -euo pipefail
if [[ $# -lt 2 || $# -gt 4 ]]; then
  echo "Usage: bash scripts/run_experiment.sh {trusted_lr015_bc|trusted_lr015_prox|trusted_lr015_risk|trusted_multi|trusted_multi_tail|trusted_lr015_tail500|trusted_lr015_early|trusted_lr015|legacy_lr015_500|legacy_lr020|trusted|high_lr|reference} GPU [SEED] [--dry-run]" >&2
  exit 2
fi
EXPERIMENT="$1"
GPU_ID="$2"
